...
近期的 EDA agent 研究有三個方向開始收斂：高階抽象能顯著提高 agent 的設計效率；production agent 必須以環境最終狀態判分，「工具有呼叫成功」不算數；OpenROAD 生態則正在把 MCP、可重播 flow graph、可觀測工具介面組成更完整的 agent-native substrate。

這讓我對 R2G 的判斷更明確：值得長期投資的是把「工程意圖」轉成可執行、可驗證、可重播的 flow；個別 LLM 或 agent framework 都可能被替換。
...
這個結果的意義超出 HLS 本身：它再次顯示 Agent 的能力高度取決於它操作的 abstraction。
...
「PrimeTime command 執行成功」與「Innovus script 沒有 exception」都只是過程。
結果要以重新讀取 artifact 後，timing、area、power、DRC、LEC 或其他 acceptance criteria 確實成立為準。
...
NVIDIA 建議 production evaluation 同時追 success rate、multiple-trial consistency、steps per success、cost per success。因此未來比較 Claude、GLM、Qwen 或不同 harness，單看 benchmark accuracy 幾乎沒有意義；要量的是「同一個 R2G engineering task，在同一套 tool/environment 下，多少成本可以穩定完成」。
...
第一，建立 R2G IR / Flow Contract。讓 design intent、constraint、acceptance criteria 與可修改範圍成為正式資料，不只是 prompt 裡的文字。

第二，建立 Engineering Eval Harness。每個重要 agent task 都必須可以從 persisted artifact clean replay，並由 deterministic EDA result 判斷成功。

第三，建立 Incremental Flow Graph。知道一個變更 invalidate 哪些 downstream state，讓 agent 可以快速 branch、experiment、compare，不必每次重新跑完整流程。
...
應該長期留下來的，是 design state、artifact lineage、engineering contract、evaluation result，以及累積下來的 trajectory。
