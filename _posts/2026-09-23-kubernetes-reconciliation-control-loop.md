...
假設一個 flow 做了四件事：建立 compute resource、掛載 volume、送出 job、最後寫入完成狀態。第三步完成後 process crash，第四步沒有執行。系統重新啟動時，應該從哪裡接？如果只是重播 script，可能重複建 volume、重複 submit job；如果選擇跳過，又可能漏掉真正未完成的工作。再進一步，如果 job 當下成功，但一小時後 node 掛掉，過去的 success code 對現在的 correctness 幾乎沒有意義。

Kubernetes 的核心選擇因此不是「讓每個 command 更可靠」，而是改寫 correctness 的定義：**系統正不正確，不由過去做過什麼決定，而由現在觀察到的狀態是否接近期望狀態決定。**

這就是 reconciliation loop 的真正價值。
...
真實 distributed system 恰好相反。node 會失效、process 會 crash、operator 會手動修改資源、network 會 timeout、cloud API 可能回傳 ambiguous result、storage attachment 可能卡在中間狀態。系統不是從 A 到 B 一次性前進，而是在外部擾動下持續偏離目標。
...
這不是 YAML 語法上的習慣，而是 fault model 的結果。
...
真正重要的不是 YAML，而是語義：即使 controller process 完全消失再重啟，它仍然可以只靠 API 中的 spec/status，加上對 LSF 與 artifact store 的重新觀察，推導「現在還缺什麼」。
...
這還沒計算 serialization、TLS、JSON/protobuf decode、etcd read、GC 與 API server CPU。真正大型 cluster 不可能靠高頻 full polling 維持反應速度。
...
它雖然接收 event，correctness 卻不應依賴「每個 event 必須恰好收到一次」。event 的角色更像 interrupt：提醒 controller 某個 key 值得重新檢查。controller 真正做決策之前，仍然應讀取最新 state。
...
這就是 level-triggered control 與 edge-triggered workflow 的本質差別。
...
## Reflector、cache、workqueue：目的不是快，而是把 failure 變成正常路徑
...
workqueue 的本質不是「排隊」而已，而是把每個 resource 的進度隔離，使 transient failure 可以局部重試，且錯誤率上升時自動降低 retry pressure。
...
換句話說，controller 不是追求 exactly-once action，而是追求 **repeatable convergence**。

這也是為什麼「reconcile 可以被任意重跑」幾乎是 Kubernetes controller 的基本設計準則。你不需要證明某一個 event 從沒重複，也不需要持久化每一個中間 command；只要每次都能從當前 reality 重新推導安全的下一步，crash 就主要變成額外 latency，而不是 state corruption。
...
比較安全的做法是只把 key 放進 queue：`default/job-a`。真正 reconcile 時再從 informer cache 或 API 取得最新 object。於是多個快速 update 可以自然 collapse 成「處理一次最新狀態」。

這也是為什麼很多 Kubernetes controller 的核心函式看起來都像：
...
真正的 program state 不是 queue event，而是 **API object + external observable state**。
...
如果要建立 department-scale control plane，更好的方向不是把每個 Tcl step 都改成 Kubernetes Pod，而是先把高階 state 變成 explicit、queryable、versioned object：
...
到這裡很容易走向另一個誤區：既然 reconciliation 這麼強，那是不是所有 workflow engine 都應該改成 controller？
...
這個切割非常重要，因為它避免一個常見 platform mistake：拿一個 workflow engine 來假裝是整個 control plane，最後所有外部 drift、retry、manual intervention、resource lifecycle 都只能塞成更多 workflow branch；或反過來，把 controller 當成 DAG engine，導致 sequence semantics 變得難以理解。

## Optimistic concurrency：不要鎖住整個世界，而是承認 state 可能已改變

當多個 controller 或 user 可以同時修改 object，還會出現並行更新問題。

Kubernetes object 的 `resourceVersion` 不只支援 watch，也用於 concurrency control。簡化來說，你讀到 version 100，準備更新；如果另一個 actor 已先把 object 更新成 101，你基於舊版本提交的 update 可能收到 conflict。正確做法不是假設「我剛剛讀過，所以我擁有它」，而是重新讀取、重新計算，再 retry。[Kubernetes API Concepts](https://kubernetes.io/docs/reference/using-api/api-concepts/)
...
## Controller 自己也會成為負載來源，所以 control plane 必須能在過載時保住生命線
...
這也預告下一篇為什麼要專門拆 kube-apiserver：它不是「放 YAML 的 REST server」，而是整個分散式控制系統的語義入口、併發邊界與 overload boundary。

## 把這個模型放回 AI / EDA / Foundry，Kubernetes 最有價值的地方不是 Pod
...
## 真正的判準：故障發生時，系統能不能只靠「現在」重新得出正確下一步
...
這並不表示所有 transient detail 都必須塞進 Kubernetes API。相反，真正好的設計會很克制：只把「足以重新推導 correctness」的 durable state 保存下來；大量 ephemeral event、trace、log 仍放在 observability system。

因此 reconciliation 不是把所有事情狀態化，而是把**決定系統是否正確所需的最小狀態**明確化。
