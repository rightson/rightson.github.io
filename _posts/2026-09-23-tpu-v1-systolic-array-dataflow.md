...
上一篇談的是 Google 為什麼必須做 TPU；這一篇要回答更底層的問題：**為什麼把 65,536 個 MAC 放在晶片上，並不自然等於 92 TOPS？真正決定效率的，是能不能讓這些 MAC 每個 cycle 都拿得到資料，而且不要為每一次 multiply-add 都回 SRAM 或 DRAM 取數。**
...
這就是 systolic array 的價值。它不是一種「更多乘法器」的設計，而是一種**用空間上的資料重用，交換通用處理器的控制與搬移成本**的設計。
...
但如果這 65,536 個 MAC 每個 cycle 都各自從 SRAM 讀兩個 operand，再把結果寫回 SRAM，記憶體埠數、wire、energy 與面積會先失控。Google 對第一代 TPU 的描述很直接：systolic array 讓一次讀進來的資料在相鄰 ALU 間繼續流動，而不是反覆經過 register file 或大型 SRAM。
...
TPU v1 最值得注意的並不是它有一條 matrix multiply instruction，而是這條 instruction 的 operand shape 幾乎直接揭露硬體。
...
這不是一般 CPU ISA 的思路。CPU 的 instruction 描述一次 scalar/vector operation；TPU 的 instruction 描述一整塊 tensor computation。軟體看到的是 `MatrixMultiply(B)`，硬體看到的則是：256-wide activation stream 應該持續餵入一個已經載好 weights 的 256×256 array。

TPU v1 的 dataflow 可以粗略理解成「一個 weight tile 在陣列中停留一段時間，activation rows 持續流過它」。嚴格說來，Google 的公開資料描述的是 weights 預先載入、input 從另一方向流入；用現代 accelerator 術語看，它具有明顯的 weight-stationary 特徵，但這裡更重要的不是分類名稱，而是**同一組 weights 能被 B 個 input rows 重複使用**。
...
這說明 systolic array 的 peak throughput 從來都不是 array size 一個數字。真正的 throughput 是：
...
這也是為什麼 accelerator 與 compiler 不能分開看。當 hardware 固定為 256×256，compiler 的工作就不只是「把 TensorFlow operator 換成 TPU instruction」，而是必須處理 tiling、padding、layout、buffer placement，以及哪一段 computation 值得留在 MXU、哪一段交給其他單元。
...
因此，理解 TPU v1 的 systolic array，不應停在「Google 用了 256×256 MAC」。真正值得記住的是：**Google 把 matrix multiplication 重新表達成一條資料重用流水線，讓 operand 儘量留在晶片內、沿著鄰近 MAC 移動；而當這個方法成功後，整個系統的瓶頸就從 arithmetic 轉移到 memory bandwidth。**

下一篇會沿著這個因果鏈繼續往下：為什麼 TPU v1 願意把主資料路徑壓到 INT8？低 precision 並不只是模型壓縮技巧，它直接決定一顆晶片能放多少 MAC、SRAM 要多寬、memory bandwidth 可以服務多少運算，以及最終每瓦能換到多少 inference。
